...
exports.startLiveVideo = function startLiveVideo(options) {
  return rp({
    uri:
      "https://graph.facebook.com/v2.11/me/live_videos" +
      "?access_token=" +
      options.accessToken,
    method: "POST",
    json: options,
  }).then(response => {
    return exports.getLiveVideo({
...
if (parameter.length === 2 || parameter.length === 3) {
  switch (parameter[1]) {
    ...
    case "get_all":
      exports
        .getAllLiveVideos({ accessToken: parameter[0] })
        .then(response => {
          console.log(response)
        })
        .catch(error => {
    ...
    case "delete_all":
      exports
        ...
        .then(response => {
          ...
          return Promise.map(response.data, video =>
            exports.deleteLiveVideo({
              accessToken: parameter[0],
              postId: video.id,
            })
          ).then(responses => {
