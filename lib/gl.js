    ...
    return gl({
      ...
      frag: `
          ...
          void main() {
            ...
            vec3 koColor = clamp(keyColor + multiSmall, 0., 1.);
            float webcamKeyColor = chromaKeyAlphaTwoFloat(knockoutC,
               koColor, 1. - clamp(keyTolerance + multiSmall, 0., 1.),
              clamp(keySlope, 0.001, 1.)
            );

            vec3 webcamMixedColor = mix(c0, c1, webcamKeyColor);
