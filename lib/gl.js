  ...
  const mergeStreams = props => {
    ...
    return gl({
      ...
      frag: `
          ...
          float chromaKeyAlphaTwoFloat(vec3 color, vec3 keyColor, float keyAmount, float keySlope)
          {
            float d = abs(length(abs(keyColor - color)));
            float edge0 = keyAmount * (1.0 - keySlope);
            float alpha = smoothstep(edge0, keyAmount, d);
            return 1. - alpha;
          }
          ...
          void main() {
            ...
            float ff = chromaKeyAlphaTwoFloat(knockoutC, koColor, 1. - clamp(keyAmount + multiSmall, 0., 1.), clamp(keySlope, 0.001, 1.));
