  ...
  ;(function(window) {
    ...
    jsmpeg.prototype.decodePicture = function(skipOutput) {
      ...
      if (skipOutput != DECODE_SKIP_OUTPUT) {
        ...
        this.outBuffer = pixels*/
        //this.outBuffer = toBuffer(pixels)

        //FFMPEG.frame(toBuffer(pixels))

        /*var rawImageData = {
            data: toBuffer(pixels),
            width: this.width,
            height: this.height,
          }

          //var jpegImageData = jpeg.encode(rawImageData, 50)
          let jpeg = new libjpeg.Jpeg(
            toBuffer(pixels),
            this.width,
            this.height,
            "rgba"
...
*/
        ...
        /*let jpeg = new libjpeg.Jpeg(toBuffer(pixels), W, H, "rgba")
          jpeg.encode((image, err) => {
            console.log(image)
          })*/
        //console.log(jpeg);
        ...
        //var encoded = jpg.compressSync(toBuffer(pixels.buffer), options)

        //var preallocated = new Buffer(jpg.bufferSize(options))
        //console.log(pixels.buffer);

        //var encoded = jpg.compressSync(toBuffer(pixels), preallocated, options)

        /*var encoded = jpg.compressSync(
            toBuffer(pixels),
            new Buffer(jpg.bufferSize(options)),
            options
          )*/
        //console.log(encoded);

        //          console.log(canvas.ImageData(pixels))
        //ctx.putImageData(new Canvas.ImageData(pixels), 0,0);

        //var img = new Image;
        //console.log(img.onload);
        /*img.onload = (err) => {
            console.log(this.address, "LODED");
          }*/
        //img.src = toBuffer(pixels)
        //ctx.drawImage(img,0,0, 640, 480)
        /*ctx.toDataURL('image/jpeg', function(err, jpeg){
            console.log(jpeg);
           }); // sync JPEG is not supported*/
        //FFMPEG.frame(toBuffer(pixels))

        //FFMPEG.frame(jpeg.encodeSync())
        //console.log(toBuffer(pixels));
        //FF.frame(jpegImageData.data)
        //FF.frame(new libjpeg.Jpeg(toBuffer(pixels), this.width, this.height, "rgba"))
