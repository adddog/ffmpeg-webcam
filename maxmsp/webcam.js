...
;(function(window) {
  ...
  var jsmpeg = (window.jsmpeg = function(url, opts) {
    ...
    if (!opts.forceCanvas2D && this.initWebGL()) {
      ...
      this.canvasContext = this.canvas.getContext("max-2d")
  ...
  jsmpeg.prototype.initSocketClient = function(client) {
    ...
    // Setup Audio Decoder
    this.audioInputBuffer = new BitReader(
      new ArrayBuffer(this.socketBufferSize)
    )
    ...
    if (this.audioCtx) {
      if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
        ...
        if (this.unlockAudioElement) {
          this.unlockAudioElement.addEventListener(
            "touchstart",
            this.unlockWebAudio.bind(this),
            false
          )
        }
      } else {
  ...
  jsmpeg.prototype.receiveSocketMessage = function(event) {
    ...
    if (this.currentPacketType == START_PACKET_VIDEO) {
      ...
      this.audioInputBuffer.bytes.set(
        messageData,
        this.audioInputBuffer.writePos
      )
      this.audioInputBuffer.writePos += messageData.length

      if (
        this.audioInputBuffer.writePos >= this.currentPacketLength
      ) {
        var bytes = new Uint8Array(
          this.audioInputBuffer.bytes.buffer,
          8,
          this.currentPacketLength
        )
        ...
        if (this.audioCtx.sampleRate !== 44100) {
          if (!this.resampler) {
            var resampleBufferLength =
              decodedChunk.length * (this.audioCtx.sampleRate / 44100)
            this.resampler = new Resampler(
              44100,
              this.audioCtx.sampleRate,
              1,
              resampleBufferLength
            )
          }
          this.decodedAudioBuffer = this.concatFloat32Arrays(
            this.decodedAudioBuffer,
            this.resampler.resample(decodedChunk)
          )
        } else {
          this.decodedAudioBuffer = this.concatFloat32Arrays(
            this.decodedAudioBuffer,
      ...
      if (
        !this.webAudioUnlocked &&
        this.unlockAudioElement &&
        !this.unlockAudioElementVisible
      ) {
  ...
  jsmpeg.prototype.decodeSequenceHeader = function() {
    ...
    if (this.buffer.getBits(1)) {
      ...
      for (var i = 0; i < 64; i++) {
        this.customIntraQuantMatrix[ZIG_ZAG[i]] = this.buffer.getBits(
          8
        )
      }
  ...
  jsmpeg.prototype.initWebGL = function() {
    ...
    gl.attachShader(
      this.program,
      this.compileShader(gl.VERTEX_SHADER, SHADER_VERTEX_IDENTITY)
    )
    gl.attachShader(
      this.program,
      this.compileShader(
    ...
    gl.attachShader(
      this.loadingProgram,
      this.compileShader(gl.FRAGMENT_SHADER, SHADER_FRAGMENT_LOADING)
    )
...
var canvas = new MaxCanvas(this)
var ctx = canvas.getContext("max-2d")
var example
...
var connect = function() {
  ...
  player = new jsmpeg(client, { canvas: canvas, unlockAudio: false })
