...
const WIDTH = 352
const HEIGHT = 288
const AUDIO_INPUT_CHANNEL = ":0"
// ffmpeg -f avfoundation -list_devices true -i ""
const VIDEO_DIR = "_used"
//!!!!!!!!
const USE_OMX = false
const PIPE_FFPLAY = true
const NO_OVERLAY_VIDEO = false
const OFFLINE = true
...
const WEBCAM_IPS = ["192.168.1.154, 192.168.1.160"] //, "10.0.1.3"//, "10.0.1.7"
...
const startFFMPEG = rtmpUrl => {
  const _videoBitrate = ` -preset ultrafast -tune zerolatency  -b:v ${BITRATE_V}k -minrate ${BITRATE_V /
    ...
    NO_AUDIO
      ? " -an  -analyzeduration 32 -probesize 32 "
  ...
  let _format = OFFLINE ? `${TCP ? "" : " -f mpegts"} ` : ` -f flv `
  if (IS_YOUTUBE) {
    _format = "  -f flv "
  }
